...
function getISO3166ByPhone(phone) {
	...
	for (var i = 0; i < iso3166_data.length; i++) {
		...
		var country_name_len = iso3166_obj.country_code.length;
		var phone_length_list = iso3166_obj.phone_number_lengths;

		// check if phone macth the country_code & phone length
		if (phone.match(regex) && phone_length_list.indexOf(phone.length - country_name_len) > -1) {
...
function validatePhoneISO3166(phone, iso3166) {
	...
	var phone_length_list = iso3166.phone_number_lengths;
	if (phone_length_list.indexOf(phone.length) > -1) {
